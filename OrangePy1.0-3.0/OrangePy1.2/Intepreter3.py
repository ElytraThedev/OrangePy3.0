...
class OrangePy:
    ...
    def parse(self, command):
        ...
        elif command.lower().startswith("while "):
            self.loop(command[6:])
        else:
    ...
    def loop(self, statement):
        if " do " in statement:
            condition, actions = statement.split(" do ", 1)
            condition = condition.strip()
            actions = actions.strip().split(' Var ')
            ...
            print(f"{RED}SyntaxError: Invalid while statement format{RESET}")
